...
import ni.org.ics.zip.movil.controller.zpAgendaStudioController;
...
/**
 * Created by FIRSTICT on 11/23/2016. V1.0
 */
...
public class AgendaService {
	...
	public List<ZpAgendaEmbarazada> getDiary(String fecha) throws ParseException {
		...
		Query query = session.createQuery(
				"select zp00 from Zp00Screening zp00 where zp00.recordId not in ( select zp08.recordId from Zp08StudyExit zp08 )");
		screenings = query.list();

		for (Zp00Screening screening : screenings) {
			fechaIngreso = DateUtil.StringToDate(DateUtil.DateToString(screening.getScrVisitDate(), "dd/MM/yyyy"),
					"dd/MM/yyyy");
			// QUINCENALES
			// visita semana2
			...
			if (entrySectionFtoK != null && entrySectionFtoK.getSeaNextDate() != null) {
				fechaAcordada = DateUtil.StringToDate(
						DateUtil.DateToString(entrySectionFtoK.getSeaNextDate(), "dd/MM/yyyy"), "dd/MM/yyyy");
				...
				if (fechaAcordada.compareTo(fechaValidar) == 0) {
					...
					agendaEmbarazada.setHoraCita(DateUtil.DateToString(DateUtil.StringToDate(
							fecha + " " + entrySectionFtoK.getSeaNextTime(), "dd/MM/yyyy HH:mm:ss"), "hh:mm a"));
					agendaEmbarazada.setFormulario(Constants.ENTRY_LABEL);
					// Obtener fecha de visita que se encuentra en la primera parte del formulario
					// (AtoD)
					...
					if (entryAtoD != null) {
						agendaEmbarazada
								.setFechaProgramacionCita(DateUtil.DateToString(entryAtoD.getSeaVdate(), "dd/MM/yyyy"));
					}
					agendaEmbarazadas.add(agendaEmbarazada);
				}
			}
			...
			query = session.createQuery(
					"from Zp03MonthlyVisit zp03 " + "where zp03.recordId = :codigo and zp03.redcapEventName = :event ");
			query.setParameter("codigo", screening.getRecordId());
			...
			if (monthlyVisit != null && monthlyVisit.getMonNextDate() != null) {
				fechaAcordada = DateUtil
						.StringToDate(DateUtil.DateToString(monthlyVisit.getMonNextDate(), "dd/MM/yyyy"), "dd/MM/yyyy");
				...
				if (fechaAcordada.compareTo(fechaValidar) == 0) {
					...
					agendaEmbarazada.setFechaProgramada(DateUtil.DateToString(fechaProgramada, "dd/MM/yyyy"));
					agendaEmbarazada.setFechaCita(DateUtil.DateToString(fechaAcordada, "dd/MM/yyyy"));
					agendaEmbarazada.setHoraCita(DateUtil.DateToString(
							DateUtil.StringToDate(fecha + " " + monthlyVisit.getMonNextTime(), "dd/MM/yyyy HH:mm:ss"),
							"hh:mm a"));
					agendaEmbarazada.setFormulario(Constants.MONTHLY_VISIT + " 1");
					agendaEmbarazada.setFechaProgramacionCita(
							DateUtil.DateToString(monthlyVisit.getMonVisitDate(), "dd/MM/yyyy"));
					agendaEmbarazadas.add(agendaEmbarazada);
				}
			}
			// visita semana12
			...
			if (trimesterVisitSectionFtoHs != null) {
				for (Zp04TrimesterVisitSectionFtoH trimesterVisit : trimesterVisitSectionFtoHs) {
					if (trimesterVisit.getTriNextVisitDat() != null) {
						fechaAcordada = DateUtil.StringToDate(
								DateUtil.DateToString(trimesterVisit.getTriNextVisitDat(), "dd/MM/yyyy"), "dd/MM/yyyy");
						if (fechaAcordada.compareTo(fechaValidar) == 0) {
							int semana = DateUtil.getWeek(screening.getScrVisitDate(),
									trimesterVisit.getTriNextVisitDat());
							agendaEmbarazada = new ZpAgendaEmbarazada();
							agendaEmbarazada.setCodigo(screening.getRecordId());
							agendaEmbarazada.setEvento(String.valueOf(semana) + " " + Constants.WEEK_LABEL);
							agendaEmbarazada
									.setFechaIngreso(DateUtil.DateToString(screening.getScrVisitDate(), "dd/MM/yyyy"));
							agendaEmbarazada.setFechaProgramada("");
							agendaEmbarazada.setFechaCita(DateUtil.DateToString(fechaAcordada, "dd/MM/yyyy"));
							agendaEmbarazada.setHoraCita(DateUtil.DateToString(
									DateUtil.StringToDate(fecha + " " + trimesterVisit.getTriNextVisitTime(),
											"dd/MM/yyyy HH:mm:ss"),
									"hh:mm a"));
							...
							if (tvAtoD != null) {
								agendaEmbarazada.setFechaProgramacionCita(
										DateUtil.DateToString(tvAtoD.getTriDov(), "dd/MM/yyyy"));
							}
							agendaEmbarazadas.add(agendaEmbarazada);
						}
					}
	...
	public List<ZpAgendaEstudio> getCitas(Date fechaFiltro, String tipoCita) {
		...
		Query query = session.createQuery(
				"from ZpAgendaEstudio as  z where z.appointmentDateTime >= :fecha and z.appointmentDateTime < :fechaF");
	...
	public List<ZpAgendaEstudio> getCitas(Date fechaInicio, Date fechaFin, String tipo_agenda, String unidad_salud) {
		...
		String qst = "from ZpAgendaEstudio as  z where z.appointmentDateTime >= :fecha "
				+ "and z.appointmentDateTime <= :fechaF " + " and (z.healtUnit = :unidad or :unidad  = '')"
	...
	// Metodo que regresa el listado de citas por usuario, esto es para la apicacion
	// movil
	@SuppressWarnings("unchecked")
	public List<ZpAgendaEstudio> getCitasPorUsuario(String username) {

		try {
			List<UserCenter> centrosusuario = this.usuarioService.getCentersUser(username);
			List<String> listaCentros = new ArrayList<String>();
			for (UserCenter us : centrosusuario)
				listaCentros.add(us.getUserCenterId().getCenter());

			List<ZpAgendaEstudio> agenda = new ArrayList<ZpAgendaEstudio>();

			Session session = sessionFactory.getCurrentSession();
			String qst = "from ZpAgendaEstudio as  z where z.healtUnit in (:names) ";

			Query query = session.createQuery(qst);
			query.setParameterList("names", listaCentros);

			agenda = query.list();
			return agenda;
		} catch (Exception ex) {
			logger.info("Error al intentar recuperar la lista de citas por usuario. " + ex.getMessage());
			return null;
		}
	...
	public ZpAgendaEstudio getZpAgendaEstudio(String tipoAgenda, Date fechahora) {
		...
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery(
				"FROM ZpAgendaEstudio p WHERE p.appointmentType=:tipoagenda AND p.appointmentDateTime = :fecha");
