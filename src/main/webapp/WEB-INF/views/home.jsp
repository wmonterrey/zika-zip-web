...
<spring:url
	value="/resources/plugins/bootstrap-daterangepicker/daterangepicker-bs3.css"
	var="daterangecss" />
<link rel="stylesheet" type="text/css" href="${daterangecss}" />
<spring:url value="/resources/plugins/data-tables/DT_bootstrap.css"
	var="dtbootcss" />
<link rel="stylesheet" href="${dtbootcss}" />
<spring:url
	value="/resources/plugins/data-tables/TableTools/css/dataTables.tableTools.css"
	var="dtttcss" />
...
<body class="page-header-fixed page-sidebar-fixed page-footer-fixed">
							...
							<ul class="page-breadcrumb breadcrumb">
								<li><i class="fa fa-home"></i> <a
									href="<spring:url value="/" htmlEscape="true "/>"><spring:message
											code="home" /></a> <i class="fa fa-angle-right"></i></li>
								<li class="pull-right">
									<div id="dashboard-report-range"
										class="dashboard-date-range tooltips" data-placement="top"
										data-original-title="<spring:message code="heading" />">
							...
							<div id="tamizajes-dia-div" class="portlet">
								<div class="portlet-title">
									<div class="caption">
										...
										<spring:message code="screenxday" />

									</div>
									<div class="actions">
										<a href="#" onclick="exportTamizajesDia();"
								...
								<div class="portlet-body">
									...
									<table class="table table-striped table-hover table-bordered"
										...
										<c:forEach items="${tamizajesDia}" var="tamizajeDia">
											<tr>
												<td><fmt:formatDate value="${tamizajeDia[0]}"
	...
	<!-- Date Range-->
	<spring:url
		value="/resources/plugins/bootstrap-daterangepicker/moment.min.js"
		var="DateRangeMoment" />
	<script src="${DateRangeMoment}" type="text/javascript"></script>
	<spring:url
		value="/resources/plugins/bootstrap-daterangepicker/daterangepicker.js"
		var="DateRange" />
	...
	<spring:url value="/resources/plugins/flot/jquery.flot.resize.js"
		var="jQFlotResize" />
	<script type="text/javascript" src="${jQFlotResize}"></script>
	<spring:url value="/resources/plugins/html2canvas/html2canvas.js"
		var="html2Canvas" />
	<script src="${html2Canvas}"></script>
	<spring:url value="/resources/plugins/flot/jquery.flot.categories.js"
		var="categories" />
	...
	<c:set var="processSuccess">
		<spring:message code="process.success" />
	</c:set>
	<c:set var="processError">
		<spring:message code="process.error" />
	</c:set>
	<c:set var="deniedError">
		<spring:message code="denied" />
	</c:set>
	<c:set var="yesterday">
		<spring:message code="yesterday" />
	</c:set>
	<c:set var="today">
		<spring:message code="today" />
	</c:set>
	<c:set var="last7days">
		<spring:message code="last7days" />
	</c:set>
	<c:set var="last30Days">
		<spring:message code="last30Days" />
	</c:set>
	<c:set var="tMonth">
		<spring:message code="tmonth" />
	</c:set>
	<c:set var="lMonth">
		<spring:message code="lmonth" />
	</c:set>
	<c:set var="applyLabel">
		<spring:message code="applyLabel" />
	</c:set>
	<c:set var="fromLabel">
		<spring:message code="fromLabel" />
	</c:set>
	<c:set var="toLabel">
		<spring:message code="toLabel" />
	</c:set>
	<c:set var="customRangeLabel">
		<spring:message code="customRangeLabel" />
	</c:set>
	<c:set var="Su">
		<spring:message code="Su" />
	</c:set>
	<c:set var="Mo">
		<spring:message code="Mo" />
	</c:set>
	<c:set var="Tu">
		<spring:message code="Tu" />
	</c:set>
	<c:set var="We">
		<spring:message code="We" />
	</c:set>
	<c:set var="Th">
		<spring:message code="Th" />
	</c:set>
	<c:set var="Fr">
