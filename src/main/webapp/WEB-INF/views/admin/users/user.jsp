...
<body class="page-header-fixed page-sidebar-fixed page-footer-fixed">
	...
	<div class="page-container">
		...
		<!-- BEGIN CONTENT -->
		<c:set var="userEnabledLabel">
			<spring:message code="login.userEnabled" />
		</c:set>
		<c:set var="userDisabledLabel">
			<spring:message code="login.userDisabled" />
		</c:set>
		<c:set var="userLockedLabel">
			<spring:message code="login.accountLocked" />
		</c:set>
		<c:set var="userUnlockedLabel">
			<spring:message code="login.accountNotLocked" />
		</c:set>
		<c:set var="habilitar">
			<spring:message code="enable" />
		</c:set>
		<c:set var="deshabilitar">
			<spring:message code="disable" />
		</c:set>
		<c:set var="bloquear">
			<spring:message code="lock" />
		</c:set>
		<c:set var="desbloquear">
			<spring:message code="unlock" />
		</c:set>
		<c:set var="confirmar">
		...
		<spring:url value="/admin/users/lockunl/lock2/{username}"
			var="lockUrl">
			<spring:param name="username" value="${user.username}" />
		</spring:url>
		<spring:url value="/admin/users/lockunl/unlock2/{username}"
			var="unlockUrl">
			<spring:param name="username" value="${user.username}" />
		</spring:url>
		<div class="page-content-wrapper">
			<div class="page-content-wrapper">
				<div class="page-content">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<h3 class="page-title">
								...
								<small><spring:message code="adminusers" /></small>
							</h3>
							<ul class="page-breadcrumb breadcrumb">
								<li><i class="fa fa-home"></i> <a
									href="<spring:url value="/" htmlEscape="true "/>"><spring:message
											code="home" /></a> <i class="fa fa-angle-right"></i> <a
									href="<spring:url value="/admin/users/" htmlEscape="true "/>"><spring:message
											code="adminusers" /></a> <i class="fa fa-angle-right"></i> <a
									href="<spring:url value="/admin/users/${user.username}" htmlEscape="true "/>">${user.username}</a>
								</li>
							</ul>
								...
								<div class="portlet-body">
									<div class="table-toolbar">
										<div class="btn-group">
											...
											<button id="lista_usuarios_new"
												onclick="location.href='${fn:escapeXml(listUser)}'"
												class="btn btn-info">
												<spring:message code="adminusers" />
										...
										<div class="btn-group pull-right">
											...
											<ul class="dropdown-menu pull-right">
												<li><a href="${fn:escapeXml(editUrl)}"><i
														class="fa fa-edit"></i> <spring:message code="edit" /></a></li>
												<li><a href="${fn:escapeXml(chgpassUrl)}"><i
														class="fa fa-key"></i> <spring:message code="changepass" /></a>
												</li>
												<c:choose>
													<c:when test="${user.enabled}">
														<li><a data-toggle="modal"
															data-id="${fn:escapeXml(disableUrl)}" class="desact"><i
																class="fa fa-trash-o"></i> <spring:message
																	code="disable" /></a></li>
													</c:when>
													<c:otherwise>
														<li><a data-toggle="modal"
															data-id="${fn:escapeXml(enableUrl)}" class="act"><i
																class="fa fa-check"></i> <spring:message code="enable" /></a>
														</li>
													</c:otherwise>
												</c:choose>
												<c:choose>
													<c:when test="${user.accountNonLocked}">
														<li><a data-toggle="modal"
															data-id="${fn:escapeXml(lockUrl)}" class="lock"><i
																class="fa fa-lock"></i> <spring:message code="lock" /></a>
														</li>
													</c:when>
													<c:otherwise>
														<li><a data-toggle="modal"
															data-id="${fn:escapeXml(unlockUrl)}" class="unlock"><i
																class="fa fa-unlock"></i> <spring:message code="unlock" /></a>
														</li>
													</c:otherwise>
												</c:choose>
											</ul>
						...
						<div class="col-md-12">
							...
							<div class="portlet">
								<div class="portlet-title">
									<div class="caption">
										<i class="fa fa-group"></i>
										<spring:message code="usercenters" />
									</div>
									<div class="tools">
										<a href="javascript:;" class="collapse"></a> <a
											href="javascript:;" class="remove"></a>
									...
									<div class="table-responsive">
										<table class="table table-striped table-hover table-bordered"
											...
											<c:forEach items="${centrosusuario}" var="centro">
												<tr>
													<td><spring:message
															code="${centro.userCenterId.center}" /></td>
													<c:choose>
											...
											<c:forEach items="${accesses}" var="acceso">
												<tr>
													<td class="hidden-xs"><c:out
															value="${acceso.sessionId}" /></td>
													<td class="hidden-xs"><c:out
															value="${acceso.remoteIpAddress}" /></td>
													<td><c:out value="${acceso.loginDate}" /></td>
													<td><c:out value="${acceso.logoutDate}" /></td>
													<td class="hidden-xs"><c:out
															value="${acceso.logoutRefererUrl}" /></td>
												</tr>
											</c:forEach>
										</table>
									</div>
							...
							<div class="portlet">
								<div class="portlet-title">
									<div class="caption">
										...
										<spring:message code="audittrail" />
									</div>
									<div class="tools">
										<a href="javascript:;" class="collapse"></a> <a
											href="javascript:;" class="remove"></a>
					...
					<!-- END ROW -->
					<div class="modal fade" id="basic" tabindex="-1" data-role="basic"
						data-backdrop="static" data-aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal"
										data-aria-hidden="true"></button>
									<div id="titulo">
